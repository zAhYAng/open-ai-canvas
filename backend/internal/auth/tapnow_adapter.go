...
func adaptTapNowDetail(detail *tapNowDetail, shareID string) (*TapNowImportResult, error) {
	...
	if !isTapNowShareID(shareID) {
		return nil, kernel.BadAuthRequest("TapNow 分享 ID 格式无效")
	}
	...
	for _, raw := range detail.Nodes {
		...
		item := TapNowCanvasNode{
			...
			Height:        height,
			Content:       content,
			Prompt:        kernel.FirstNonEmpty(strings.TrimSpace(data.Prompt), firstTapNowParam(data.Params, "prompt")),
		...
		item.MimeType = inferTapNowMime(kind, content, kernel.FirstNonEmpty(data.Prompt, data.Title))
