...
AutoDL 的模型选择对应 ComfyUI 工作流 ID。不同工作流可以声明不同参数，插件清单中的工作流贡献负责描述参数、默认值和画布能力。当前内置示例使用 `minimax_h3_lightx2v_no_pic`，表示 MiniMax H3 文生视频工作流；另已内置 `minimax_h3_z0901` 文生视频、`minimax_h3_z0902` 六图生视频与 `minimax_h3_z0903` 六图三音频生视频。后续新增工作流只需增加工作流贡献，不需要修改生成服务代码。
