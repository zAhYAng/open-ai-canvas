...
func TestClearAdminRuntimeCacheRejectsUnknownScopeAndAuditsLocalClear(t *testing.T) {
	svc, db := newSystemPerformanceTestService(t)
	admin := &model.User{ID: "admin-1", Role: model.UserRoleAdmin}
	svc.coordinator.localRate["user-1"] = localRateEntry{started: time.Now(), count: 2}
	...
	if len(svc.coordinator.localRate) != 1 {
	...
	if result.DeletedKeys != 1 || len(svc.coordinator.localRate) != 0 {
		t.Fatalf("local clear result = %#v, remaining = %d", result, len(svc.coordinator.localRate))
	}
...
func TestClearAdminRuntimeCacheUsesRedisWhitelist(t *testing.T) {
	...
	svc, db := newSystemPerformanceTestService(t)
	svc.coordinator = &runtimeCoordinator{redis: client, instanceID: "system-performance-test", localRate: map[string]localRateEntry{}, localSlots: map[string]map[string]time.Time{}}
...
func newSystemPerformanceTestService(t *testing.T) (*Service, *gorm.DB) {
	...
	return &Service{
		...
		dataDir:              dataDir,
		coordinator:          &runtimeCoordinator{instanceID: "local-test", localRate: map[string]localRateEntry{}, localSlots: map[string]map[string]time.Time{}},
