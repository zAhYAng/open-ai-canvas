...
import type { CanvasOperation, CanvasSnapshot } from "./canvas-operation-contract";
...
export function buildCanvasWorkflowOps(input: CanvasWorkflowInput, snapshot: CanvasSnapshot, config: AiConfig, identityPrefix?: string): CanvasOperation[] {
...
function layoutWorkflowNodes(nodes: CanvasWorkflowNodeInput[], snapshot: CanvasSnapshot, direction: "horizontal" | "vertical", gap: number, start?: { x: number; y: number }) {
...
export function workflowNodeIdsFromOps(ops: CanvasOperation[]) {
    return ops.filter((op): op is Extract<CanvasOperation, { type: "add_node" }> => op.type === "add_node" && Boolean(op.id)).map((op) => op.id!);
}
