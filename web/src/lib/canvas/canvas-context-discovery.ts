import { hashCanvasSnapshot, type CanvasOperation, type CanvasSnapshot } from "./canvas-operation-contract";
...
export function buildCanvasContext(snapshot: CanvasSnapshot, options: { stateHash?: string; hashSource?: "browser-local" | "canvas-server" } = {}) {
    ...
    return {
        ...
        // The cloud control plane calculates the canonical hash on the server.
        // The browser fingerprint is marked explicitly so it cannot be mistaken
        // for the server hash in a write precondition.
        stateHash: options.stateHash || hashCanvasSnapshot(snapshot),
...
export function findCanvasNodes(snapshot: CanvasSnapshot, input: { query?: string; ids?: string[]; types?: string[]; statuses?: string[]; resourceOnly?: boolean; limit?: number }) {
...
export function getCanvasNode(snapshot: CanvasSnapshot, input: { id: string }) {
...
export function getCanvasConnection(snapshot: CanvasSnapshot, input: { id: string }) {
...
export function getCanvasGenerationTasks(snapshot: CanvasSnapshot, input: { status?: string; nodeIds?: string[]; limit?: number }) {
...
export function getCanvasResources(snapshot: CanvasSnapshot, input: { nodeIds?: string[]; status?: string; limit?: number }) {
    const nodeIds = input.nodeIds?.length ? new Set(input.nodeIds) : null;
...
export function validateCanvasOperations(snapshot: CanvasSnapshot, ops: CanvasOperation[]) {
    ...
    return { ok: !issues.some((issue) => issue.severity === "error"), issues, operationCount: ops.length, currentStateHash: hashCanvasSnapshot(snapshot) };
}
...
function resourceFromNode(node: CanvasNodeData): CanvasResourceInsight | null {
    const metadata = (node.metadata || {}) as Record<string, unknown>;
