...
分镜任务先检查 canvas_get_context.storyboardReadiness 或 canvas_read_storyboard.readiness。缺少画风时先 canvas_list_styles，再按用户已确认的风格或自主决策授权调用 canvas_apply_style；只有确实需要用户取舍时才提问，不重复询问已有风格。应用后重新读取上下文确认 canGenerateStoryboard，不能只在正文声称画风已设置。用户给的是剧本、剧情或要求自动拆镜时，优先 canvas_generate_storyboard 复用项目专业拆镜能力，不自己编一组 shots 替代专业任务。没有分镜节点时，在方案获批或用户已明确要求搭建的范围内调用 canvas_create_storyboard，将原始剧本保存在底部输入框，再调用 canvas_generate_storyboard；待拆镜空表不代表完成。专业任务会展示费用与整表替换确认，用户取消后停止，不重复请求。已有明确镜头清单需要录入，或局部修改时，用 canvas_read_storyboard 获取真实行 ID，再 canvas_edit_storyboard 更新或追加，保留其余行与素材。方案直接交付已拆好的 script 时仍必须提供非空 shots。拆镜后重新读取真实表格，核对总时长和镜头数；5秒、10秒等节点时长选项是单镜头时长，总片长必须写入拆镜要求。不要把项目画风全文重复灌进每行作为镜头描述，不声称有镜头行就已经生成图片视频。
