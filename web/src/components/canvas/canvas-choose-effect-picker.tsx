...
import { FEED_TABS, toAbsoluteUrl } from "@/lib/canvas/canvas-tool-presentation";

const EFFECT_TOOL_PAGE_SIZE = 40;
...
export function CanvasChooseEffectPicker({
    ...
    const toolsQuery = useInfiniteQuery({
        queryKey: ["canvas-tools", userId, "effect-picker", scopeTab, searchText],
        queryFn: ({ signal, pageParam }) => listTools({ page: pageParam, pageSize: EFFECT_TOOL_PAGE_SIZE, scope: scopeTab, type: "effect", search: searchText || undefined }, { signal }),
        initialPageParam: 1,
